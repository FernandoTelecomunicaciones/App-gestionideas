...
### D-33 UI milestone (M3–M7, M10): what was built and where it deviates from the written architecture
The user-facing app was built natively in Compose + M3 from `/design`, PRODUCT_SPEC §5 and ARCHITECTURE §8/§9. The reminder engine was not touched. Every deviation from the docs is listed here; none changes a product rule.

| # | Docs said | Built | Why |
|---|---|---|---|
| 1 | M3 `NavigationBar` with a transparent indicator (ARCH §13) | Custom row on `selectable(Role.Tab)` inside `selectableGroup()`, 56 dp, 2 px top rule | M3's bar is 80 dp with an indicator pill; the design's bar is ~55 px. Same semantics for TalkBack. |
| 2 | `SingleChoiceSegmentedButtonRow` for priority (EDT-06) | One single-choice group made of the square `AhoraChip` (radio role); segments equal width and height; stacked when `fontScale > 1.3` | The M3 segmented button cannot be forced to a 48 dp target with wrapping labels at 200 % font scale. Behaviour is identical (tapping the selected segment clears it). |
| 3 | `Repetir` / `Duración estimada` as selects (prototype) | Wrapping rows of the same chips | 48 dp targets, no popup, everything visible at once. |
| 4 | ViewModels expose `StateFlow<UiState>` only (ARCH §4) | Screens' UiState is a `StateFlow`, but **typed text and the editor draft are Compose snapshot state** (`mutableStateOf`) mirrored synchronously into `SavedStateHandle` | ARCH §9.2 already requires text to update synchronously (no caret jumps); a `StateFlow` collected on the frame dispatcher can lag a frame. `EditorViewModel`, `InboxViewModel`, `TasksViewModel.query`. |
| 5 | Snackbar ≈ 2.2 s for messages without an action (PRODUCT_SPEC §9.3, DD-8) | Platform `Short` for **all** messages | A manual timer would bypass the system accessibility timeouts. Messages with Deshacer were already `Short`. |
| 6 | `MainActivity.pendingDestination` state, consumed by the host | A `Channel<LinkDestination>(UNLIMITED)` collected once by `AhoraRoot` | Two identical taps are two events; nothing is lost before the first frame; no "clear after consume" protocol. Validation is still `NotificationLinkHandler` (revision checks untouched). |
| 7 | "clear editor/navigation state" (R2-8) for notification entry | A notification body/ABRIR link **dismisses an open editor sheet** (the draft is discarded) before navigating | A modal sheet would hide the destination. A half-typed draft is lost; deliberate and cheap to change. |
| 8 | Hoy's "También pendiente" collapses because bottom-bar navigation to Home uses `restoreState = false` (R2-13) | The flag lives in the Hoy `SavedStateHandle` (survives rotation/process death) and `HomeRoute` collapses it from a `NavController` destination-changed listener when the destination is not Hoy | Hoy is the root of the stack and never leaves it, so `restoreState=false` cannot reset it. Config changes do not fire the listener, so rotation keeps it. |
| 9 | Foco session = preset + end timestamp in `SavedStateHandle` (R2-12) | Stored as ONE `LongArray [preset, endMillis]`; a session change restarts the ticker | Found by a test: two keys exposed "15 min" with the old end time for an instant, and a stale tick showed up to 1 s of drift. |
| 10 | "Completadas" as a discreet link (TSK-05) | Right-aligned in the "Tareas" title row | Reachable without scrolling; still not a section or chip. |
| 11 | Snackbar/undo tokens resolved "by the ViewModel" | `TaskActions` (singleton, application scope) runs complete/postpone/delete/create/undo and posts the message; ViewModels call it | Terminar leaves Foco immediately, so the write must not depend on the ViewModel that started it. It contains no business rule (the repository decides everything). |
| 12 | Import "clears editor state" | Import runs from Ajustes where no sheet can be open; the repository's `replaceAll` already resets alarms and notifications | Nothing to clear; noted so R2-8 stays traceable. |

**Backup file (Ajustes › Datos).** `BackupCodec` (pure Kotlin, `domain/backup`): `{schemaVersion: 1, exportedAt, tasks[]}`. User data only — reminder delivery state (`fired`, `snooze`, `revision`) is not exported, so an imported reminder starts fresh and the 12 h rule stops a stale replay. Strict decode: unknown `schemaVersion` → "otra versión"; unknown recurrence, priority ∉ 1..3, unparseable date/time, seconds in a time, duplicate ids, blank/untrimmed title, title > 1 000 / notes > 20 000 / list > 200 chars, > 50 000 tasks, > 8 MiB → whole file rejected, nothing written. Import is two-step (validate → dialog with both counts → `replaceAll`); the repository validates again and resets every reminder.

**Not built (deliberately):** subtasks, tags, projects, statistics, streaks, any additional setting. The only settings are the approved ones (permissions, theme, keep-screen-on in Foco, export/import).

### D-34 Gate C amendments (supported dates, shared text limits, committed-but-not-replanned, channel-aware notifications)
Normative text: ARCHITECTURE §20. The reminder engine's design is unchanged; these are integration and boundary fixes.
- **Supported date range 1900-01-01 … 2200-12-31** (`TaskRules.MIN_DATE/MAX_DATE`). It contains the editor's date picker (1900–2100). Outside it `java.time` still parses but the engine cannot follow (`Instant.toEpochMilli()` overflows when arming, `plusDays` overflows when a recurring task is completed). Import rejects a due date or a recurrence anchor outside it; `normalize` drops an unsupported date together with its dependents (time, reminder, repeat). *Gave up:* dates before 1900 / after 2200.
- **One set of text limits for every write path and the backup file** (`TaskRules.MAX_TITLE 1 000 / MAX_NOTES 20 000 / MAX_LIST 200`). The editor and quick capture stop typing there and `normalize` is the backstop, so an export always re-imports; export fails visibly if the file would exceed the 8 MiB import limit. *Gave up:* notes longer than 20 000 characters.
- **A failed re-plan after a commit is not a failed write.** `TaskRepositoryImpl` catches (and logs, no task text) an exception from `ReminderSync` after the Room commit; the alarm is a rebuildable cache and the next receiver/app start reconciles (§10.4). Before, a failure there made `create` report "No se pudo guardar" for a row that existed and dropped the Undo token. **Amends R2-3:** the repository still awaits the re-plan, but its failure no longer propagates. `TaskActions` also catches every write it runs in the application scope (which has no handler) and shows the generic "No se pudo guardar" instead of ending the process.
- **`undoComplete` refuses a successor the user has edited** (`updatedAt != createdAt`), the same "the series has moved on" outcome as D-31. *Gave up:* Deshacer no longer removes an occurrence that was edited in the ≈ 4 s Snackbar window.
- **"Can notify" includes the reminders channel** (`AndroidReminderNotifier.canPostReminders`): a channel set to `IMPORTANCE_NONE` is "notifications not allowed" for the engine (consumed silently, PRODUCT_SPEC §8.1) and for Ajustes/the editor hint. `NotificationManagerCompat.areNotificationsEnabled()` alone misses it.
- Small UI fixes: `openEdit` is superseded by any later open/dismiss (a slow read can no longer reopen the sheet over Foco or overwrite another draft); Hoy/Mañana chips resolve the date when tapped, not when composed (a sheet left open across midnight); `AhoraSwitch` carries an accessible name and honest state words ("Activado/Desactivado", not "Permitidas").
- CI now also runs `assembleRelease` (R8 breaks were invisible to it).

### D-35 Release identity, versioning and signing (release prerequisites, 2026-09-21)
**Identity (verified against the build, not changed):** `applicationId` = `namespace` = `com.fernando.ahora`, launcher label **AHORA** (OD-1 resolution above).

**First release: `versionName "0.1.0"`, `versionCode 1`.** Policy:
- `versionName` is `MAJOR.MINOR.PATCH`. `0.x` = test/pre-release builds; `1.0.0` is the first build the owner calls stable. PATCH = fixes only, MINOR = user-visible behaviour, MAJOR is reserved.
- `versionCode` is a plain integer that goes **+1 for every build that is distributed** (sideloaded APK, GitHub release, store upload) and is **never reused or lowered**: Android refuses to install a lower code over a higher one, and stores reject a repeat. It is *not* bumped per commit. It is edited by hand in `app/build.gradle.kts` in the same commit as the `versionName` change, and a `vMAJOR.MINOR.PATCH` git tag marks that commit (no tag exists yet). Re-signing or re-uploading the *same* bytes keeps the code; any different installable APK gets a new one.
- Deliberately **not** a formula derived from `versionName` (`0.1.0` would give 100, not the 1 the owner specified).

**Signing.** One dedicated production release keystore, held by the owner, **never in the repository, in CI or in logs.** `app/build.gradle.kts` creates `signingConfigs.release` only when all four values are supplied, from the git-ignored `keystore.properties` (repository root) or from `AHORA_KEYSTORE_FILE` / `AHORA_KEYSTORE_PASSWORD` / `AHORA_KEY_ALIAS` / `AHORA_KEY_PASSWORD`. With none of them `assembleRelease` still succeeds and produces an **unsigned** APK (this is what CI builds); with only some of them the build **fails**, naming the keys and never a value, so an APK cannot silently ship unsigned. `.gitignore` covers `*.jks`, `*.keystore`, `*.p12`, `*.pfx`, `keystore.properties`. A release signed with the debug key is a smoke test only, never a release artifact. Procedure and the exact items the owner supplies: [RELEASE.md](RELEASE.md).
**Gave up:** signing in CI (the key never leaves the owner's machine); automatic version numbers. **Accepted consequence:** the same key must sign every future update (same package + different signature = no in-place update), so the keystore needs an offline backup.

### D-36 Release-validation tooling and the first accessibility fix (2026-09-21)
- `scripts/device_matrix.py` (standard-library Python + adb) is the repeatable device matrix: launch, permission state, navigation, quick capture, Room persistence across force-stop, edit, Hoy/Foco, the `POST_NOTIFICATIONS` dialog and the denied case, exact-alarm-allowed and exact-alarm-denied delivery, real reminder delivery with body/actions, HECHO, +10 MIN, ABRIR after process death, body tap on a warm app, reboot recovery with the app never opened, and an unnamed/under-48 dp accessibility audit at 100 % and 200 % font. It installs the **debug** APK (it needs the adb `DebugCommandReceiver`), so it validates the platform behaviour of the engine and UI, not the R8 build. Results and how to run it: [RELEASE.md](RELEASE.md).
- `scripts/release_smoke.py` is the counterpart for the **release** APK: real UI only (the release has no debug hook), four real reminders created through the editor and the time dial, delivery/actions/cold ABRIR/reboot/snooze re-delivery, plus signature fingerprint and installed-package checks. It is the test to run once the owner's key exists; `--allow-debug-key` is for rehearsals and a debug-signed run is never the final result. Rehearsed on API 36 with the R8 build: 15/15.
- **A11y defect fixed:** the editor sheet's drag handle was passed to M3 through `dragHandle`, which wraps it in a clickable node that TalkBack reached as an *unlabelled 48×35 dp button* (found by the audit; below the 48 dp rule and unnamed). The handle is now drawn inside the sheet and `dragHandle = null`; the whole sheet still drags and dismisses, the layout is unchanged, and the audit reports 0 unnamed / undersized controls on the editor.
- CI: the workflow makes `gradlew` executable (it was committed from Windows with mode `100644`, which would fail with "Permission denied" on the Linux runner), sets `permissions: contents: read` and a 30-minute timeout. It needs no secrets. The `ubuntu-latest` image documents `android-37.0` and build-tools `37.0.0` as preinstalled, so no SDK download is required. **It has still never run on GitHub.**
...
## Gate C — Codex review of the complete working tree (2026-09-21)

Run: `codex-companion task` (fresh, read-only, `--effort high`) over commit `04cedc8` (clean tree): all of `app/src`, manifests, R8 rules, backup rules, schema, tests, CI and docs, with the 10-area checklist (A–J) and 10 known items. Result as reported: **0 CRITICAL, 12 IMPORTANT, 1 OPTIONAL**; verdict "not release-approvable" (chiefly because of the device/TalkBack/signing gaps). Every finding was re-verified against the code before acting; **severity below is mine**. Codex independently confirmed as clean: emergency retry cursor, `MAX_IMPORT_REVISION`, recurring-`reopen` refusal, request conflation, snooze idempotence, transaction boundaries, no destructive migrations, only `MainActivity` exported, no `INTERNET`, cloud backup excluded, debug receiver debug-only, R8 consumer rules, API 26 compatibility.

| ID | Codex | My class | Verified | Resolution |
|---|---|---|---|---|
| GC-01 | IMPORTANT | **Rejected — not reproducible** | Reading: plausible only if the system relaunches a process-dead task with the *new* intent as the launch intent and a non-null saved state. **Device test** (Android 16 emulator, task retained in Recents, process killed twice, Ajustes as the saved screen): ABRIR → Foco, back → Hoy; body → Hoy with Ajustes gone. The intent arrives through `onNewIntent`, which the listener registered in `onCreate` handles. | None. Kept as a device-matrix row for other API levels. |
| GC-02 | IMPORTANT | **OPTIONAL** (rejected) | Real in principle (check-then-cancel by task id), but the window is microseconds AND needs a schedule edit whose new reminder fires inside it. A lock in the notifier would not close it: `notify()` is asynchronous in the system server, so `activeNotifications` can still show revision N after N+1 was enqueued. The real fix is revision-specific notification identity — an engine refactor. | None; the GB-01 KDoc already states the residual window. |
| GC-03 | IMPORTANT | **IMPORTANT** (lower end) | Yes: a blocked channel silently drops `notify()`; the reminder is consumed and Ajustes said "Permitidas". Spec-consistent outcome, wrong signal. | D-34; 2 tests. |
| GC-04 | IMPORTANT | **IMPORTANT** (my own review found it first) | Yes: `dueDate = +999999999-12-31` imports fine; completing a daily one throws in the application scope; a reminder on it overflows `toEpochMilli` in `arm`. Needs a crafted file, hence not CRITICAL. | D-34; 5 tests. |
| GC-05 | IMPORTANT | **IMPORTANT** | Yes: only `TaskActions.create` caught; the rest ran uncaught in a handler-less scope; a post-commit re-plan failure surfaced as a failed save. | D-34; 4 tests. |
| GC-06 | IMPORTANT | **OPTIONAL** (fixed: 3 lines) | Yes, but the window is the ≈ 4 s Snackbar. | D-34; 2 tests. |
| GC-07 | IMPORTANT | **OPTIONAL** (fixed: 6 lines) | Yes: uncancelled read publishes late. Sub-frame in practice. | D-34; 4 tests (fail without the fix). |
| GC-08 | IMPORTANT | **IMPORTANT** | Yes: `AhoraSwitch` had a role and a state but no name (and used "Permitidas" for a toggle). | D-34; 2 tests; accessibility tree inspected on the emulator (same structure as the labelled checkboxes). **Not** verified with TalkBack. |
| GC-09 | IMPORTANT | **IMPORTANT** | Yes: 20 001 characters of notes saved and exported, but the backup was rejected on import. | D-34; 4 tests. |
| GC-10 | OPTIONAL | **OPTIONAL** (rejected) | Yes: a failure after truncation leaves a partial file in a document the user chose; the app already reports failure. Atomic replace over SAF is provider-dependent. | None (acceptable limitation). |
| GC-11 | IMPORTANT | **IMPORTANT, split** | Yes: no Activity-level test of `handleLink`/root collector/receivers; CI lacked `assembleRelease`. | CI runs `assembleRelease` (fixed). Activity/instrumented tests: **not added** (Gate C is a hardening pass; behaviour was verified on the device instead) — remaining risk. |
| GC-12 | IMPORTANT | **Release packaging (M12), not a code defect** | Yes: `assembleRelease` is unsigned; signing needs the owner's key. | Not done — belongs to Gate D / APK release. |
| GC-13 | IMPORTANT | **OPTIONAL** (fixed: 2 lines) | Yes: Hoy/Mañana captured at composition. | D-34; no automated test (composable-level; verified by reading). |

**Known items (Codex + mine).** Editor draft discarded by a link: *acceptable limitation* (modal must close; the reopen race is GC-07, fixed). Foco landscape scroll to Salir: *acceptable limitation / optional polish* (system back is the same action). Unused Lucide icons and the other lint warnings: *optional polish*. Android 16 emulator only, no physical device, no TalkBack, CI never run: *pre-release blockers for the device matrix and A11Y-04*, not code defects. No screenshot tests: *important hardening, deferred*. Gate B fixes re-reviewed: emergency cursor, revision validation, reopen restriction, conflation, snooze idempotence clean; GB-01 residual window accepted (GC-02); GB-04 undo guard extended (GC-06).
